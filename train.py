...
for episode in tqdm(range(episodes_to_run)):
    ...
    while not done:
        ...
        # print(" Action Logits : ", action_logits.size(), action_logits)
        ...
        # print("observation tensor shape: ", observation_tensor)
        # print(" observation shape: ", observation_tensor.shape)
    ...
    per_time_step_losses = [-log_prob * total_reward_ for log_prob in action_log_probs]
    per_time_step_losses_t = torch.stack(per_time_step_losses)
    loss = per_time_step_losses_t.sum()
    ...
    print('episode ', episode, ' total reward' , total_reward , " Loss : %.4f " % loss.item())
...
torch.save(model.state_dict(), 'model/own-model-doom-5k.pth')
